...
public abstract class DeviceWaitingSearch extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            while (isRunning) {
                // 等待主机的搜索
                Logger.d("Client: 等待主机进入搜索...");
                socket.receive(pack); // 这里要做修改，不能阻塞
                if (verifySearchData(pack)) {
                    ...
                    Logger.d("Client:  给主机回复信息");
                    socket.send(sendPack);
                    Logger.d("Client:  等待主机接收确认");
                    socket.setSoTimeout(RECEIVE_TIME_OUT);
                    try {
                        socket.receive(pack);
                        if (verifyCheckData(pack)) {
                            Logger.d("Client:  确认成功");
    ...
    private boolean verifyCheckData(DatagramPacket pack) {
        ...
        Logger.d("Client:  ip from host=" + ip);
